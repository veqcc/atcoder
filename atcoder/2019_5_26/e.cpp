...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        int l = vec[i].second.first - vec[i].first;
        int r = vec[i].second.second - vec[i].first;

        int idx1 = lower_bound(D.begin(), D.end(), l) - D.begin();
        int idx2 = lower_bound(D.begin(), D.end(), r) - D.begin();

        seg.update(idx1, idx2, vec[i].first);
    }
